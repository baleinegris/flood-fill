...
# Function to get elevation data for a given latitude and longitude using Google Maps API
def _get_elevation(lat, lon):
    ...
    if response.status_code == 200:
        # Parse the JSON response to get elevation data
        results = response.json().get('results')
        if results:
            return results[0].get('elevation')
    # Return None if the request was not successful or no elevation data was found
    return None
